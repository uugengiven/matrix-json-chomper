...
const start = () => {
    ...
    if(filename) 
    {
        generateOutput(filename, roomName);
    }
    else
    {
...
const generateOutput = (fileName, roomName) => {
    let rawdata = fs.readFileSync(`./input/${fileName}`);
    let info = JSON.parse(rawdata);

    const team = "default"
    const room = roomName ? roomName.replaceAll(" ", "-").toLowerCase() : info.room_name.replaceAll(" ", "-").toLowerCase();
